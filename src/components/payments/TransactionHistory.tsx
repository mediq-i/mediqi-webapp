...
function TransactionHistory() {
  ...
  const { data:paymentHistory } = useUserQuery({
    queryCallback: () =>
      PaymentAdapter.getPaymentHistory({ id: data?.user?.id }),
    queryKey: [queryKeys.PAYMENT_HISTORY],
  });
  console.log("payments",paymentHistory)
        ...
        <TableBody>
          {transactions.map((transaction, index) => (
            <TableRow key={index} className="h-[72px]">
              ...
              <TableCell className="">
                <p className="border rounded-lg flex items-center">
                  <Dot
                    className={
